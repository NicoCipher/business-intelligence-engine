...
As of schema v9, a Problem carries two independent current-state fields
describing its trajectory: `lifecycle_state` (is it operationally
relevant right now — new, active, dormant, archived, or reactivated)
and `trend` (how is its evidence cadence changing — unknown, growing,
stable, or declining). These were deliberately kept separate rather than
combined into one state, for the same reason described in the Historical
Evolution section below: one field, one concept. Confidence remains part
of the existing opportunity scoring model and is not part of either
field.
